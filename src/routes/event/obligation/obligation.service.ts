...
import { EventInfoType, EventQuery } from './model/obligationModel';
import * as moment from 'moment';

export default class ObligationService {
  static async create(data: EventInfoType) {
    const nowTime = moment().format('YYYY-MM-DD HH:mm:ss');
    if (data.id) {
      const id = data.id;
      delete data.id;
      const dbData = await prisma.obligation.update({
        where: { id },
        data: {
          ...data,
    ...
    const dbData = await prisma.obligation.create({
      data: {
        ...data,
        updated_time: nowTime,
        create_time: nowTime,
  ...
  static async getLimit({ page, limit, id }: EventQuery) {
    ...
    const participateIn = await prisma.obligationRegistration.findMany({
      where: {
        obligation_id: {
  ...
  static async findOne({
    ...
    const data = await prisma.obligation.findUnique({
      where: {
        id: eventId,
      },
    });
    if (participateInId) {
      const result = await prisma.obligationRegistration.findUnique({
        where: {
          id: participateInId,
        },
      });
  ...
  static async delete(id: number) {
    const data = await prisma.obligation.delete({
      where: {
        id,
      },
    });
  ...
  static async join({ id, userId }: { id: number; userId: number }) {
    const isHas = await prisma.obligationRegistration.findFirst({
      where: {
        user_id: userId,
        obligation_id: id,
      },
    ...
    if (!isHas) {
      const result = await prisma.obligationRegistration.create({
        data: {
          user_id: userId,
          obligation_id: id,
        },
  ...
  static async cancel(id: number) {
    try {
      const result = await prisma.obligationRegistration.delete({
        where: {
          id,
        },
      });
