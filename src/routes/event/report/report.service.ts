...
import { EventInfoType, EventQuery } from './model/reportModel';
...
export default class ReportService {
  static async create(data: EventInfoType) {
    ...
    if (data.id) {
      ...
      delete data.id;
      data.people_count;
      const dbData = await prisma.report.update({
        where: { id },
        data: {
          ...data,
    ...
    const dbData = await prisma.report.create({
      data: {
        ...data,
        updated_time: nowTime,
        create_time: nowTime,
  ...
  static async getLimit({ page, limit, id }: EventQuery) {
    ...
    const participateIn = await prisma.reportRegistration.findMany({
      where: {
        report_id: {
  ...
  static async findOne({
    ...
    if (participateInId) {
      const result = await prisma.reportRegistration.findUnique({
        where: {
          id: participateInId,
        },
      });
  ...
  static async join({ id, userId }: { id: number; userId: number }) {
    const isHas = await prisma.reportRegistration.findFirst({
      where: {
        user_id: userId,
        report_id: id,
      },
    ...
    if (!isHas) {
      const result = await prisma.reportRegistration.create({
        data: {
          user_id: userId,
          report_id: id,
        },
  ...
  static async cancel(id: number) {
    const result = await prisma.reportRegistration.delete({
      where: {
        id,
      },
    });
